...
app.directive('startupError', function($compile) {
	...
	return {
		...
		link : function(scope, iElement, iAttrs) {
			...
			$(mensagem).attr('ng-show',
					iAttrs.formName + '.' + campo.attr('name') + '.$invalid && ' + iAttrs.formName + '.' + campo.attr('name') + '.$dirty');
	...
	return {
		...
		link : function(scope, iElement, iAttrs) {
			...
			var select = function(event, jsEvent, view) {
				...
				if (event != null && event.movimentacao != null) {
					scope.functionSelect(event.movimentacao);
				}
			};
